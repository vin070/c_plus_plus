...
template <typename T> bool search_in_bst(binary_tree<T> *root, T const &k) {
  if (!root)
    return false;

  if (k == root->get_data())
    return true;
  else if (k > root->get_data())
    return search_in_bst(root->get_right_node(), k);
  else
    return search_in_bst(root->get_left_node(), k);
}
...
int main() {
  ...
  int search_data = 0;
  std::cin >> search_data;
  std::cout << (search_in_bst(root, search_data) ? "true" : "false");
  delete root;
  return 0;
}