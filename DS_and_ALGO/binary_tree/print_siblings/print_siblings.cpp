...
template <typename T> void print_nodes_without_sibling(binary_tree<T> *root) {
  if (!root)
    return;

  if (root->get_right_node() && !root->get_left_node())
    std::cout << root->get_right_node()->get_data() << " ";
  else if (root->get_left_node() && !root->get_right_node())
    std::cout << root->get_left_node()->get_data() << " ";

  print_nodes_without_sibling(root->get_left_node());
  print_nodes_without_sibling(root->get_right_node());
}
int main() {
  binary_tree<int> *root = binary_tree<int>::take_input_levelwise();
  print_nodes_without_sibling<int>(root);
  delete root;
  return 0;
}