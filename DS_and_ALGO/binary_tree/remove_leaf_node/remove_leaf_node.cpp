...
template <typename T> binary_tree<T> *remove_leaf_nodes(binary_tree<T> *root) {
  if (root && !root->get_left_node() && !root->get_right_node()) {
    delete root;
    return NULL;
  } else if (!root)
    return NULL;

  root->update_left_node(remove_leaf_nodes(root->get_left_node()));
  root->update_right_node(remove_leaf_nodes(root->get_right_node()));
  return root;
}

int main() {
  binary_tree<int> *root = binary_tree<int>::take_input_levelwise();
  root = remove_leaf_nodes<int>(root);
