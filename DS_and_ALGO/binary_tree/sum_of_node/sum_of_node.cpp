...
template <typename T>
T get_sum(binary_tree<T>* root) {
   if(!root) return 0;
    
    return get_sum(root->get_left_node()) + get_sum(root->get_right_node()) + root->get_data();
}

int main() {
  binary_tree<int> *root = binary_tree<int>::take_input_levelwise();
  int sum_of_node  = get_sum<int>(root);
  std::cout << sum_of_node << '\n';
  delete root;
  return 0;
}