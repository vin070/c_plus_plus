...
public:
  ...
  static int length_recursively(linked_list *head) {
    if (head == NULL)
      return 0;

    return 1 + linked_list::length_recursively(head->get_next());
  }
...
int main() {
  ...
  while (test_case--) {
    ...
    head = linked_list::delete_recursively(head, 0, delete_index);
