...
linked_list *getListAfterReverseOperation(linked_list *head, int const &b_size,
  ...
  int ele_to_skip = 0;
  linked_list *subset_head = head;
  linked_list *prev_subset_tail = NULL;
  for (int i = 0; i < b_size; ++i) {
    ...
    linked_list *subset_tail = subset_head;
    for (int j = 0; j < b[i] - 1 && subset_tail->get_next(); ++j)
      subset_tail = subset_tail->get_next();
    ...
    if (i == 0)
      head = ans;
    if (prev_subset_tail)
      prev_subset_tail->set_next(ans);
    subset_head->set_next(subset_tail_next);
    prev_subset_tail = subset_head;
    subset_head = subset_tail_next;
  }
  return head;
}

int main() {
  int block_reverse_size = 0;
  linked_list *head = linked_list::take_input();
  std::cin >> block_reverse_size;
  int *block_reverses = new int[block_reverse_size];
