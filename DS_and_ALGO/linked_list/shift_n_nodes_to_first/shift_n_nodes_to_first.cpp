...
linked_list *append_last_n_to_first(linked_list *head, int const& n) {
  int len = linked_list::length(head);
  linked_list *new_head = head;
  linked_list *new_tail = NULL;
  linked_list *prev_tail = NULL;
  linked_list *temp = head;

  for (int i = 0; i < len - 1; ++i) {
    if (i == len - n - 1) {
      new_head = temp->get_next();
      new_tail = temp;
    }
    temp = temp->get_next();
  }
...
int main() {
  ...
  while (test_cases--) {
    ...
    int nodes_to_shift_right = 0;
    std::cin >> nodes_to_shift_right;
    head = append_last_n_to_first(head, nodes_to_shift_right);
