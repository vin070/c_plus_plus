...
int get_large_node_count(tree_node<int> *root, int const &x) {
  if (!root)
    return 0;

  int greater_nodes = root->get_data() > x ? 1 : 0;

  for (int i = 0; i < root->child.size(); ++i)
    greater_nodes += get_large_node_count(root->child.at(i), x);

  return greater_nodes;
}
...
int main() {
  ...
  std::cin >> lower_limit;
  std::cout << get_large_node_count(root, lower_limit) << std::endl;
  return 0;
}