...
bool are_identical(tree_node<int> *root1, tree_node<int> *root2) {
  if (!root1 || !root2 || root1->get_data() != root2->get_data() ||
      root1->child.size() != root2->child.size())
    return false;

  for (int i = 0; i < root1->child.size(); ++i) {
    return are_identical(root1->child.at(i), root2->child.at(i));
  }
  return true;
}
int main() {
  tree_node<int> *root1 = tree_node<int>::take_input_levelwise();
  tree_node<int> *root2 = tree_node<int>::take_input_levelwise();
  std::cout << (are_identical(root1, root2) ? "true" : "false");
  return 0;
}