...
public:
  ...
  max_node(tree_node<T> *node, T child_sum) {
    this->node = node;
    this->child_sum = child_sum;
  }
};

template <typename T> max_node<T> *max_sum_node(tree_node<T> *root) {
  if (!root) {
    max_node<T> *ans = new max_node<T>();
    return ans;
  };

  int child_sum = 0;
  for (int i = 0; i < root->child.size(); ++i)
    child_sum += root->child.at(i)->get_data();

  max_node<T> *ans = new max_node<T>(root, child_sum);
  for (int i = 0; i < root->child.size(); ++i) {
    max_node<T> *small_ans = max_sum_node(root->child.at(i));
    if (small_ans->child_sum > ans->child_sum) {
      delete ans;
      ans = small_ans;
    }
  }
...
tree_node<int> *maxSumNode(tree_node<int> *root) {
  return max_sum_node<int>(root)->node;
}

int main() {
  tree_node<int> *root = tree_node<int>::take_input_levelwise();
  std::cout << maxSumNode(root)->get_data() << std::endl;
  return 0;
}